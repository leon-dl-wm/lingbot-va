...
4. [训练 Loss](#4-训练-loss)
5. [资源占用](#5-资源占用)
6. [Checkpoint 与评测结果](#6-checkpoint-与评测结果)(含 6.5 与论文 benchmark 对比)
...
- **未做**:RoboTwin 仿真 SR 数字(需 sapien+vulkan 仿真环境,见 7.4 节评估)

## 6.5 与论文 Benchmark 结果对比

### 论文报告的 RoboTwin 2.0 结果(50 任务平均,官方 50K 步 posttrain 权重)
...
| π₀.₅ | 82.7 | 76.8 |
| Motus | 88.7 | 87.0 |
| **LingBot-VA(论文)** | **92.9 (+4.2)** | **91.6 (+4.6)** |

### 论文报告的 LIBERO 结果(官方 4-5K 步 posttrain 权重)
...
| **LingBot-VA(论文)** | 98.5±0.3 | 99.6±0.3 | 97.2±0.2 | 98.5±0.5 | **98.5** |

### 本次复现与论文的差距分析

| 维度 | 论文 | 本次复现 | 差距 |
|---|---|---|---|
| 训练步数 | **50,000**(RoboTwin) | **10,004** | 1/5 训练量 |
| 训练数据 | robotwin-clean-and-aug-lerobot | 同一数据集 | ✅ 一致 |
| 有效 batch | ≥32(官方建议) | 32 | ✅ 一致 |
| lr / 优化器 | 1e-5 / AdamW | 1e-5 / AdamW | ✅ 一致 |
| 硬件 | NVIDIA(未披露型号) | 8×BW1000 | 不同硬件,数值路径有 bf16 差异 |
| **仿真 SR 数字** | 92.9 / 91.6 | **未测** | 需 RoboTwin 仿真环境(见 7.4) |
| 定性评测 | — | i2va demo(5K/10K)✅ | 已有,可对比生成质量 |

### 结论与对齐路径

1. **训练配置与论文完全对齐**(数据/batch/lr/优化器),唯一差距是步数(10K vs 50K)——这是有意的分阶段决策
2. **SR 数字未测**:受 sapien 渲染限制(7.4 节),需 NVIDIA 机器跑仿真客户端 + BW1000 跑推理 server 才能出数
3. **预期**:10K 步(1/5 训练量)的 SR 会低于论文 92.9;从 loss 看 latent 仍在下降,续训到 50K 是对齐论文数字的必要条件
4. **建议顺序**:先用现有 10K checkpoint 跑一轮仿真评测(拿到基线 SR)→ 决定是否投入 ~25 天续训到 50K → 再跑最终评测对齐论文
